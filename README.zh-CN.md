...
![Mirror's Edge 存档管理器应用窗口](docs/assets/app-window.png)
...
内置预设随程序提供，不能编辑或删除，并单独显示在 **内置** 标签中，不会与 **我的预设** 中用户创建的内容混排。
...
- 导入外部 `.dat` 文件作为预设。
- 经明确确认后一次清空全部暂存历史。
- 编辑或经明确确认后永久删除用户创建的预设和暂存；内置预设始终只读。
- 在预设和暂存之间安全切换，原副本保持不变。
- 切换前自动备份当前存档。
- 操作前后检查文件完整性，确保存档没有损坏。
- 操作中断时自动恢复，避免丢失原存档。
- 不重复存储相同的预设；任意可靠副本已保存相同内容时，也不再创建暂存。
- 从当前存档捕获副本时记录最后应用来源和应用后变化状态，便于识别暂存历史。
- 在当前存档上显示管理器最后应用的来源，以及当前内容是否仍与应用时一致。
- 提供四个常用起点的只读内置预设。
- 支持英文和简体中文，选择会自动保存。
...
2. 打开管理器，先查看 **Current**，确认最后应用来源及当前内容是否已在应用后发生变化。
3. 点击 **Save as Stash** 留一份备份，或点击 **Save as Preset** 保存一个常用起点；检查带时间的预填名称、按需填写描述，然后确认。
...
6. 在 **Stash** 标签中找回历史状态，也可以用 **Make Preset** 把暂存变成预设。
   用户创建的预设和暂存还可以重命名、修改描述，或在确认后永久删除。使用
   **Clear Stash** 并再次确认，可以清空全部暂存历史，而不会改变 Current 和预设。
...
**部分汉字缺失** —— 如果使用英文版 Windows，请安装简体中文语言支持或简体中文
补充字体，然后重新启动管理器；Windows 显示语言可以继续保持英文。管理器的中文界面
会优先使用 Microsoft YaHei UI（微软雅黑），不可用时由 Slint 使用系统字体回退。

## 更新与移除

本程序是独立可执行文件。更新前请关闭游戏和所有正在运行的管理器实例，再用新版
可执行文件替换旧版。Stored Save 和设置会继续保留在 LocalAppData 中。如果新版不
支持已有的持久化数据结构，程序会明确报告不受支持的数据，而不会擅自改写或丢弃。

删除可执行文件不会删除 Stored Save。若要同时移除全部程序数据，请先完成或处理
界面报告的恢复操作，另外保留仍需要的副本，关闭管理器，然后手动删除
`%LOCALAPPDATA%\Mirror's Edge Save Manager\`。
